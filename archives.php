			...
			<?php if ( have_posts() ) : ?>
				...
				<header class="page-header">
					<h1 class="page-title archives-title">
						<?php
							if ( is_category() ) :
								printf( __( '%s', 'able' ), '<span>' . single_cat_title( '', false ) . '</span>' );
							...
							elseif ( is_author() ) :
								...
								*/
								the_post();
								printf( __( '%s', 'able' ), '<span class="vcard"><a class="url fn n" href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '" title="' . esc_attr( get_the_author() ) . '" rel="me">' . get_the_author() . '</a></span>' );
